...
namespace System.Linq
{
    ...
    public static class PagingExtensions
    {
        ...
        /// <returns>排序分页后的数据</returns>
        public static IQueryable<T> Page<T>(this IQueryable<T> query, int pageIndex, int pageSize, IDictionary<string, Direction> orderings = null)
        {
            Checker.NotNull(query, nameof(query));
        ...
        /// <param name="orderings">排序信息</param>
        /// <returns>分页后的数据</returns>
        public static PagedResult<T> ToPagedResult<T>(this IQueryable<T> source, int pageIndex, int pageSize, IDictionary<string, Direction> orderings = null)
        {
            Checker.NotNull(source, nameof(source));
            Checker.CheckMustNonNegativeInteger(pageIndex, nameof(pageIndex));
            Checker.CheckMustNonNegativeInteger(pageSize, nameof(pageSize));
            int count = source.Count();
            IList<T> result = source.Page(pageIndex, pageSize, orderings).ToList();
            return new PagedResult<T>() { Toltal = count, Result = result, };
        }
    }
}
